...
var observer = new MutationObserver(async function (mutations) {
    ...
    for (var i = 0; i < arrow_list.length; i++) {
        ...
        if (arrow.childElementCount == 0) {
            continue;
        }
        var item = arrow.children[0];
        if (item.attributes.opacity.value != 1 || item.tagName.toLowerCase() == 'circle') {
            continue;
        }
        var start_x = Math.floor(parseFloat(item.attributes.x1.value) + 4);
        var start_y = Math.floor(parseFloat(item.attributes.y1.value) + 4);
        var end_x = Math.floor(parseFloat(item.attributes.x2.value) + 4);
        var end_y = Math.floor(parseFloat(item.attributes.y2.value) + 4);
